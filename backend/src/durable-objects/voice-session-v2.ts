...
import type { ServerMessageV2, ClientMessageV2 } from '../types_v2';
import { 
  type DeepgramVoice, 
  getVoiceModelId, 
...
export class VoiceTeacherSession extends DurableObject<Env> {
  ...
  async webSocketMessage(ws: WebSocket, message: string | ArrayBuffer) {
    try {
      const data = JSON.parse(message as string) as ClientMessageV2 & {
        voice?: DeepgramVoice;
        ragContext?: string;
  ...
  private send(ws: WebSocket, msg: ServerMessageV2 | Record<string, unknown>) {
    try {
